...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-lg">
      <p className="text-sm font-medium text-slate-900">{point.fullDateLabel}</p>
      <p className="mt-1 text-sm text-slate-600">{payload[0].value} tickets created</p>
    </div>
  );
}
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-panel">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Seven-day trend generated directly from ticket creation timestamps.</p>
        </div>
